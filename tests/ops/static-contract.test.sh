...
assert_file_contains "$caddy" "img-src 'self' data:" "preview CSP must allow inline normalized private media"
...
assert_file_contains "$caddy" "private, no-store" "preview and CMS must be private/no-store"
assert_file_contains "$caddy" "header_up -X-Preview-Trusted" "client preview header must be stripped"
assert_file_contains "$caddy" 'frame-ancestors https://{$CMS_DOMAIN}' "preview framing must be limited to CMS"
...
assert_file_contains "$compose" "CONTENT_SOURCE: directus" "production preview must read Directus at runtime"
